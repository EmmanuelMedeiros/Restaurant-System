...
interface UserComponentProps {
  screenCategory: 'edit' | 'create';
  userToEdit?: IUser
}

export default function UserComponent({ screenCategory, userToEdit }: UserComponentProps) {
  ...
  const handleCreateUser = async (): Promise<void> => {
    ...
    if (!isThisTokenValid) {
      ...
      const newJwtToken: string | null = await userContext.generateJwtToken(
        refreshToken
      );
      if (!newJwtToken) {
        return router.replace("/(authentication)/login");
    ...
    const createUser: IApiResponse = await userEndpoint.create(
      userToCreate,
      jwtToken as string
    );
    if (createUser.statusCode !== 201) {
      return console.log(
        "Error while trying to create new user: " + createUser.data
      );
    } else {
      router.back();
    }
  ...
  const handleEditUser = async () => {
    ...
    const response: IApiResponse = await userEndpoint.update(jwtToken!, updateUserDTO);
    if (response.statusCode !== 200) {
      return console.log("Error while trying to create new user: " + response.data);
    }
    console.log(response.data)
    return router.replace('/(tabs)/(users)/editUser');
  };
  ...
  >
    ...
    >
      <View style={createUserStyles.boardContainer}>
        <Text style={{ textAlign: "center", marginTop: 40, fontSize: 18 }}>
          {screenCategory == 'create' ? 'CADASTRO DE USUÁRIO' : 'EDITAR USUÁRIO' }
        </Text>
        ...
        <View style={{ marginTop: 90 }}>
          ...
          >
            <ProjectSelectInput
              ...
              placeholder={ screenCategory === "create" ? 'Cargo' : userToEdit?.role!}
        ...
        <TouchableOpacity
          ...
          onPress={screenCategory === "create" ? handleCreateUser : handleEditUser}
        >
          <View
            style={{
              flexDirection: "row",
          ...
          >
            ...
            >
              {screenCategory === "create" ? 'Adicionar Usuário' : 'Editar Usuário'}
            </Text>
