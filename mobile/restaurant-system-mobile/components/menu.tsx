...
const menuCategoryList: IItemCategory[] = [
  { id: 0, title: "TODOS" },
  { id: 1, title: "ALMOÇO" },
  { id: 2, title: "TIRA-GOSTO" },
  { id: 3, title: "BEBIDAS" },
  { id: 4, title: "GUARNIÇÕES" },
];
...
export default function Menu({
  ...
  async function getItemCategory() {
    try {
      ...
      const apiResponse: IApiResponse = await itemCategoryEndpoint.getAll(
        token
      );
      if (apiResponse.statusCode !== 200) {
        console.log(`Get ALL item categories endpoint failed | err: ${apiResponse.data}`);
        setMenuCategoryList([]);
        return;
      } else {
  ...
  async function deleteCategory() {
    ...
    const apiResponse: IApiResponse = await itemCategoryEndpoint.delete(itemCategoryToShow, token);
    if (apiResponse.statusCode !== 200) {
      console.log(`Delete category endpoint failed | err: ${JSON.stringify(apiResponse.data)}`);
      return;
    }
    getItemCategory();
  }
    ...
    <View style={entireMenuStyle.container}>
      {!goBackFunction ? (
        <TouchableOpacity
          ...
          onPress={() => router.back()}
        >
          <Icons name="x" size={30} color={"#6C3232"} />
        </TouchableOpacity>
      ) : (
        <TouchableOpacity
        ...
        >
          <Icons name="chevron-left" size={30} color={"#171717"} />
        </TouchableOpacity>
      )}
      ...
      >
        {itemList
          ?.filter((x) =>
            itemCategoryToShow === 0
              ? x.category.id !== itemCategoryToShow
              : x.category.id === itemCategoryToShow
          )
          .map((element, index) => (
            <View key={index}>
              <View style={entireMenuStyle.eachItem} key={element.id}>
                <Text>R$ {Number(element.price).toFixed(2)}</Text>
                <Text
                  style={[
                    entireMenuStyle.itemName,
                    posActionItemList.find((x) => x === element)
        ...
        {orderItemList
          ?.filter((x) =>
            itemCategoryToShow === 0
              ? x.item.category.id !== itemCategoryToShow
              : x.item.category.id === itemCategoryToShow
          )
          .map((element, index) => (
            <View key={index}>
              <View style={entireMenuStyle.eachItem} key={element.item.id}>
                <View style={{ alignItems: "center", gap: 8 }}>
                  ...
                  <Text>
                    R${" "}
                    {Number(element.item.price * element.quantity).toFixed(2)}
                  </Text>
                </View>
