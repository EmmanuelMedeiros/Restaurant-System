...
export default function ProjectSelectInput({
  ...
  return (
    <SafeAreaView style={{ zIndex: 999 }}>
      ...
      >
        <Text
          style={[selectInput.inputText, { fontSize: customFontSize || 15 }]}
        >
          {selectedItem == undefined ? placeholder : selectedItem.title}
        </Text>
              ...
              <TouchableOpacity
                onPress={() => {
                  setSelectItem(item),
                    ...
                    setListClick
                      ? setListClick((prev: boolean) => !prev)
                      : null;
                }}
              >
                <Text numberOfLines={0}>{item.title}</Text>
              </TouchableOpacity>
