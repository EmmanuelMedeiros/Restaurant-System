...
export default function CreateOrder() {
  ...
  const [currentState, setCurrentState] = useState<OrderCreationStates>(
    OrderCreationStates.CREATE
  );
  const [choosenItems, setChooseItems] = useState<IItem[]>([]);
  const [preInsertOrderItems, setPreInsertOrderItems] = useState<
    CreateOrderItemDTO[]
  >([]);
  ...
  const { tableID, alreadyExistingItemsID, isInsertion, orderUUID } =
  ...
  async function getTable() {
    ...
    const apiResult: IApiResponse = await tablesEndpoint.getOne(
      Number(tableID),
      token
    );
  ...
  async function getAllItems() {
    ...
    const alreadyExistingOrderItemsID: string[] = alreadyExistingItemsID
      .toString()
      .split(",");
  ...
  async function createOrder() {
    if (currentTable) {
      ...
      const apiResponse: IApiResponse = await orderEndpoint.create(
        createOrderDTO,
        token
      );
      ...
      router.replace({
        pathname: "/(tabs)/(tables)/tableOrder",
        params: { tableID: tableID, tableName: currentTable.name },
      });
      return;
    }
  }
  ...
  async function insertToOrder() {
    if (orderUUID) {
      ...
      const apiResponse: IApiResponse =
        await orderEndpoint.manipulateOrderItems(
          String(orderUUID),
          preInsertOrderItems,
          token
        );
  ...
  useEffect(() => {
    if (currentState === OrderCreationStates.CONFIRM) {
      const orderItems: CreateOrderItemDTO[] = choosenItems.map((element) => {
        const alreadyChosenItem: CreateOrderItemDTO | undefined =
          preInsertOrderItems.find((x) => x.item === element);
